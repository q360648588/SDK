...
class ZyboxBleManager: NSObject {

    public static let shareInstance = ZyboxBleManager()
    
    /// 蓝牙开关状态
//    fileprivate(set) var blePowerState:CBManagerState = .unknown
    private var bleManager:CBCentralManager!
    private var blePowerBlock:((CBManagerState)->())?
    ...
    /// - Returns:
    func getBlePowerDidUpdateState(value:@escaping((CBManagerState)->())) {
        self.blePowerBlock = value
    }
    
    func getBlePowerState() -> CBManagerState {
        return CBManagerState.init(rawValue: self.bleManager.state.rawValue)!
    }
...
extension ZyboxBleManager:CBCentralManagerDelegate {
    // MARK: - 蓝牙开关状态
    func centralManagerDidUpdateState(_ central: CBCentralManager) {
        if let block = self.blePowerBlock {
            block(CBManagerState.init(rawValue: central.state.rawValue)!)
        }
    }
...
//        printLog("\n--------------------------------------------------------------------------\n")
    ...
    func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
        print("centralManager peripheral = \(peripheral)")
    ...
    func centralManager(_ central: CBCentralManager, didDisconnectPeripheral peripheral: CBPeripheral, error: Error?) {
        ...
        if error != nil {
            printLog("----------设备非正常断开---------- error:\(error?.localizedDescription)")
            print(".peripheral = \(ZyboxCommandModule.shareInstance.peripheral)")
...
extension ZyboxBleManager:CBPeripheralDelegate{
    // MARK: - 发现服务
