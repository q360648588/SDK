...
class ScanVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        AntCommandModule.shareInstance.matchingUUIDArray = [CBUUID.init(string: "6E400001-B5A3-F393-E0A9-E50E24DCCA9E"),CBUUID.init(string: "000001FF-3C17-D293-8E48-14FE2E4DA212")]//6E400001-B5A3-F393-E0A9-E50E24DCCA9E
        
        AntCommandModule.shareInstance.getSystemListPeripheral { modelArray in
...
//        AntCommandModule.shareInstance.getSystemListPeripheral { [weak self]modelArray in
                ...
                for item in modelArray {
                    if (item.name ?? "").lowercased().contains((self?.filterString ?? "").lowercased()) {
                        if !(self?.dataSourceArray.contains(where: { model in
                            return model as! AntScanModel == item
                        }) ?? false) {
...
extension ScanVC:UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let model:AntScanModel = self.dataSourceArray[indexPath.row] as! AntScanModel
        ...
        cell.detailTextLabel?.text = model.uuidString
...
//        AntCommandModule.shareInstance.connectDevice(peripheral: peripheral as Any) { (result) in
        AntCommandModule.shareInstance.connectDevice(peripheral: model as Any) { (result) in
            if result {
