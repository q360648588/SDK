...
@objc public class ZyBaseModule: NSObject {
    ...
    @objc dynamic public var blePowerState:CBCentralManagerState {
        return ZyBleManager.shareInstance.getBlePowerState()
    }
    ...
    @objc public func connectDevice(peripheral:Any,connectState:@escaping((Bool)->())) {
        ...
        if peripheral is String {
            ...
            if let zyModel = peripheral as? ZyScanModel {
                if let peripheral = (peripheral as! ZyScanModel).peripheral {
                    if self.peripheral != peripheral {
                        self.peripheral = peripheral
                    }
                    if #available(iOS 13.0, *) {
                        ZyBleManager.shareInstance.connect(peripheral: peripheral, options: [CBConnectPeripheralOptionRequiresANCS:true,
                                                                                                                 CBConnectPeripheralOptionNotifyOnDisconnectionKey:true,
                                                                                                               CBConnectPeripheralOptionEnableTransportBridgingKey:true,
                                                                                                                            ])
                    } else {
                        ZyBleManager.shareInstance.connect(peripheral: peripheral, options: [CBConnectPeripheralOptionNotifyOnDisconnectionKey:true])
                    }
                }else{
                    if let uuidString = zyModel.uuidString {
                        if let peripheral = ZyBleManager.shareInstance.getPeripheral(string: uuidString) {
                            if self.peripheral != peripheral {
                                self.peripheral = peripheral
                            }
                            if #available(iOS 13.0, *) {
