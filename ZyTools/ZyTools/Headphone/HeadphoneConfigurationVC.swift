//
//  HeadphoneConfigurationVC.swift
//  ZyTools
//
...
class HeadphoneConfigurationVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.dataSourceArray = [
            ...
            "\(NSLocalizedString("Date of update", comment: "更新日期")):2024-8-21",
        ]
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.currentBlePowerState = ZywlCommandModule.shareInstance.blePowerState
        ZywlCommandModule.shareInstance.bluetoothPowerStateChange { [weak self] state in
            self?.currentBlePowerState = state
        }
        
        self.currentPeripheralState = ZywlCommandModule.shareInstance.headphonePeripheral?.state
    ...
    @objc func pushNextVC() {
        ...
        let isHaveReconnect = ZywlCommandModule.shareInstance.getHeadphoneReconnectIdentifier()
        
        if isHaveReconnect.count > 0 {
            if self.navigationController?.viewControllers.count ?? 0 <= 2 {
                let vc = HeadphoneCommandVC()
                self.navigationController?.pushViewController(vc, animated: true)
            }
        }else{
            let vc = HeadphoneScanVC()
    ...
    @objc func disConnectButtonClick() {
        print("断开连接")
        ZywlCommandModule.shareInstance.disconnectHeadphone()
    }
...
extension HeadphoneConfigurationVC:UITableViewDataSource,UITableViewDelegate {
