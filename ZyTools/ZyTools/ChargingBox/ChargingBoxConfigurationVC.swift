//
//  ChargingBoxConfigurationVC.swift
//  ZyTools
//
...
class ChargingBoxConfigurationVC: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.currentPeripheralState = ZywlCommandModule.shareInstance.chargingBoxPeripheral?.state
    ...
    @objc func pushNextVC() {
        ...
        let isHaveReconnect = ZywlCommandModule.shareInstance.getChargingBoxReconnectIdentifier()
        
        if isHaveReconnect.count > 0 {
            if self.navigationController?.viewControllers.count ?? 0 <= 2 {
                let vc = ChargingBoxCommandVC()
                self.navigationController?.pushViewController(vc, animated: true)
            }
        }else{
            let vc = ChargingBoxScanVC()
...
extension ChargingBoxConfigurationVC:UITableViewDataSource,UITableViewDelegate {
