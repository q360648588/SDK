...
class WatchFactoryCommandVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        ZyCommandModule.shareInstance.reconnectDevice { [weak self] in
            if let self = self {
                let model = ZyScanModel.init()
                model.name = ZyCommandModule.shareInstance.peripheral?.name
                model.peripheral = ZyCommandModule.shareInstance.peripheral
        ...
        ZyCommandModule.shareInstance.peripheralStateChange { [weak self] state in
            if let self = self {
                if state == .disconnected {
                    self.currentDeviceString = "\(ZyCommandModule.shareInstance.peripheral?.name ?? "") 已断开"
                    
                    if ZyCommandModule.shareInstance.getReconnectIdentifier().count > 0 {
                    ...
                    self.currentDeviceString = "\(ZyCommandModule.shareInstance.peripheral?.name ?? "") 正在连接"
                }else if state == .connected {
                    self.currentDeviceString = "\(ZyCommandModule.shareInstance.peripheral?.name ?? "") 已连接"
                    let model = ZyScanModel.init()
                    model.peripheral = ZyCommandModule.shareInstance.peripheral
                    model.name = ZyCommandModule.shareInstance.peripheral?.name
                    ...
                    self.currentDeviceString = "\(ZyCommandModule.shareInstance.peripheral?.name ?? "") 正在断开"
                }
            }
    ...
    @objc func pushNextVC() {
        ...
        let vc = WatchFactoryConfigurationVC()
    ...
    @objc func connectTimeout(model:ZyScanModel) {
    ...
    func connectDevice(model:ZyScanModel) {
        ...
        ZyCommandModule.shareInstance.connectDevice(peripheral: model) { [weak self] result in
            ...
            if let self = self {
                ...
                if result {
                    ...
                    if let newNameString = userDefault.string(forKey: NewBleNameKey) {
                        if newNameString.count > 0 {
                            ZyCommandModule.shareInstance.setBleName(name: newNameString) { error in
                                if error == .none {
                        ...
                        if error == .none {
                            ...
                            if productId != nil {
                                if product != productId {
                                    if let uuidString = ZyCommandModule.shareInstance.peripheral?.identifier.uuidString {
    ...
    // MARK: - 检查升级
    func checkOtaMethod(model:ZyScanModel) {
        let userDefault = UserDefaults.standard
        let sortArray = userDefault.array(forKey: OtaSortKey)
        
        ZyCommandModule.shareInstance.checkUpgradeState { [weak self] success, error in
            print("继续升级")
            if let self = self {
                if error == .none {
                    ...
                    if success.keys.count > 0 {
                        ...
                        if fileString == nil {
                            ...
                            ZyCommandModule.shareInstance.setOtaStartUpgrade(type: Int(type) ?? 0, localFile: fileString!, isContinue: true) { progress in
    ...
    // MARK: - 升级部分
    func setOtaMethod(type:Int? = nil,model:ZyScanModel) {
        let userDefault = UserDefaults.standard
        let sortArray = userDefault.array(forKey: OtaSortKey)
        ...
        if let type = type {
            ...
            if fileString != nil {
                ...
                ZyCommandModule.shareInstance.setOtaStartUpgrade(type: type , localFile: fileString as Any, isContinue: false) { progress in
                        ...
                        if let sortArray:[Int] = sortArray as? [Int] {
                            if currentIndex < sortArray.count - 1 {
                                ...
                                DispatchQueue.main.asyncAfter(deadline: .now()+5) {
                                    if ZyCommandModule.shareInstance.peripheral?.state == .connected {
                                        self.powerOffMethod()
                                    }else{
                                        self.isNeedCheckOtaMethod = true
                                    }
    ...
    func powerOffMethod() {
        ...
        if result {
            if ZyCommandModule.shareInstance.functionListModel?.functionList_newPortocol == true {
                ZyCommandModule.shareInstance.setFactoryAndPowerOff { [weak self] error in
                    if let self = self {
                        if error == .none {
                ...
                ZyCommandModule.shareInstance.setPowerTurnOff { [weak self] error in
                    print("关机")
